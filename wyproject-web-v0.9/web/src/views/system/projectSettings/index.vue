<template>
  <div class="main">
        ...
        <el-card class="box-card cardone" v-if="showCard == '1'">
          <div slot="header" class="clearfix">
            <span style="font-size:22px"
              >{{ projectProfile.projectName }}</span
            >
            <el-button
              style="float: right"
              type="primary"
          ...
          <div class="text item">
            <template>
              <el-table :data="tableData" style="width: 100%">
                ...
                <el-table-column prop="postName" label="职位" align="center" width="128">
                  <template slot-scope="scope">
                    <div >
                ...
                <el-table-column prop="joinTime" label="加入时间" align="center" width="100">
                </el-table-column>

                <el-table-column prop="" label="开始填报" align="center" >
                  <template slot-scope="scope" >
                    <div >
                      <el-tag  v-if="scope.row.joinWithProject" >从项目创建开始</el-tag>
                      <el-tag  v-else>从加入时间开始</el-tag>
                    </div>
                  </template>
                </el-table-column>
    ...
    <el-dialog   title="开始填报时间"
                 :visible.sync="joinTypeVisible"
                 width="30%"
                 >
      <el-select v-model="joinTypeData.joinWithProject" placeholder="请选择"  style="width: 100%;" >
                        <el-option
      ...
      <span slot="footer" class="dialog-footer">
        <el-button @click="cancelJoinType">取 消</el-button>
        <el-button type="primary" @click="submitJoinType"
        >提交</el-button
        >
      </span>
...
import { removeProjectUser, updateJoinWithProject } from "@/api/system/projectSettings";
import {
  projectDetail,
  userList,
  updateHour,
...
export default {
  ...
  methods: {
    joinType (data) {
      this.joinTypeVisible = true
      this.joinTypeData.joinWithProject = data.joinWithProject
      this.joinTypeData.id = data.id
      //console.log(data);
    },

    submitJoinType () {
      if ( !this.joinTypeData.id ) {
        return this.$message.error('没有用户信息，请重新打开！')
      }

      updateJoinWithProject(this.joinTypeData).then(res =>{
        if (res.code == 200) {

          this.joinTypeVisible =false
    ...
    cancelJoinType () {
      this.joinTypeVisible = false
      this.joinTypeData.joinWithProject = ''
      this.joinTypeData.id = 0
